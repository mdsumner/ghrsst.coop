...
dsn = f"vrt://NetCDF:\"{input_path}\":analysed_sst?a_ullr=-180,89.995,180,-89.995&a_offset=25&a_scale=0.001&a_srs=EPSG:4326"
ds = gdal.Open(dsn)
opts = gdal.TranslateOptions(format = "COG", outputType = ds.GetRasterBand(1).DataType, 
...
cog_file = "gdal_analysed_sst.tif"

import pathlib
pathlib.Path(cog_file).unlink(missing_ok=True)

gdal.Translate(cog_file, ds, format = "COG", options = opts)
